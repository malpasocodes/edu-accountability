...
class DataDictionary:
    ...
    def _load_schema(self) -> None:
        ...
        for transform_name, transform_info in self.schema.get(
            "transformations", {}
        ).items():
    ...
    def validate_field_value(
        self, dataset_name: str, field_name: str, value: any
    ) -> List[str]:
        """
        ...
        if field.constraints:
            ...
            if field.constraints.min_length is not None:
                if len(str(value)) < field.constraints.min_length:
                    errors.append(
                        f"Field {field_name} must be at least {field.constraints.min_length} characters"
                    )

            if field.constraints.max_length is not None:
                if len(str(value)) > field.constraints.max_length:
                    errors.append(
                        f"Field {field_name} must be at most {field.constraints.max_length} characters"
                    )
            ...
            if field.constraints.pattern is not None:
                ...
                if not re.match(field.constraints.pattern, str(value)):
                    errors.append(
                        f"Field {field_name} must match pattern: {field.constraints.pattern}"
                    )
