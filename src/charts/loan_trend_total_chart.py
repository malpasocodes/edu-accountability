...
def _prepare_loan_trend_total_dataframe(
    ...
    *,
    ...
    aggregated["YoYChangePercent"] = aggregated["loan_dollars"].pct_change() * 100
...
def _render_loan_trend_data_table(prepared: pd.DataFrame, sector: str) -> None:
    ...
    display_data["TotalLoanDollars"] = (
        display_data["TotalLoanDollarsBillions"] * 1_000_000_000
    )
    ...
    st.caption(
        f"Annual federal loan dollar totals for {sector_label} institutions ({year_min}-{year_max}), "
...
def render_loan_trend_total_chart(
    ...
    *,
    ...
    sector_label = "4-year" if sector == "four_year" else "2-year"
    caption = (
        f"Total federal loan dollars summed across all {sector_label} institutions per year ({int(prepared['Year'].min())}-{int(prepared['Year'].max())}). "
