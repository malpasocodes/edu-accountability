...
class ValueGridSection(BaseSection):
    ...
    def render_overview(self) -> None:
        ...
        # Key insight callout
        st.info(
            "**💡 Key Insight:** This tool reveals which institutions deliver strong graduation outcomes at affordable prices—and which charge more but graduate fewer students."
        )
    ...
    def render_chart(self, chart_name: str) -> None:
        ...
        selected_filter = SessionManager.get(
            "value_grid_enrollment_filter", DEFAULT_ENROLLMENT_FILTER
        )
    ...
    def _render_value_grid_chart(
        self, label: str, dataset: pd.DataFrame, min_enrollment: int
    ) -> None:
        ...
        if missing:
            st.error(
                "Cannot render cost vs graduation chart. Missing columns: "
                + ", ".join(missing)
            )
            return
